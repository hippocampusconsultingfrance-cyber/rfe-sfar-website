...
def extract_monitorage_traumatise():
    import style
    import fiche_monitorage_traumatise as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn, _break_flag in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "monitorage_traumatise", "sections": blocks}


def extract_infarctus_myocarde():
    import style
    import fiche_infarctus_myocarde as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "infarctus_myocarde", "sections": blocks}


def extract_avc_precoce():
    import style
    import fiche_avc_precoce as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "avc_precoce", "sections": blocks}


def extract_traumatisme_cranien_grave_precoce():
    import style
    import fiche_traumatisme_cranien_grave_precoce as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "traumatisme_cranien_grave_precoce", "sections": blocks}


def extract_examens_preinterventionnels():
    import style
    import fiche_examens_preinterventionnels as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "examens_preinterventionnels", "sections": blocks}


def extract_aap_endoprotheses_coronaires():
    import style
    import fiche_aap_endoprotheses_coronaires as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "aap_endoprotheses_coronaires", "sections": blocks}


def extract_recommandations_avk():
    import style
    import fiche_recommandations_avk as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
...
def extract_tih_2002():
    import style
    import fiche_thrombopenie_induite_heparine as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "tih_2002", "sections": blocks}
...
if __name__ == "__main__":
    ...
    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique"):
            del sys.modules[mn]
    monitorage_traumatise = extract_monitorage_traumatise()
    with open(os.path.join(BUILD_DIR, "content_monitorage_traumatise.json"), "w") as f:
        json.dump(monitorage_traumatise, f, ensure_ascii=False, indent=1)
    print("monitorage_traumatise sections:", len(monitorage_traumatise["sections"]),
          "total blocks:", sum(len(s["items"]) for s in monitorage_traumatise["sections"]))

    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique", "fiche_monitorage_traumatise"):
            del sys.modules[mn]
    infarctus_myocarde = extract_infarctus_myocarde()
    with open(os.path.join(BUILD_DIR, "content_infarctus_myocarde.json"), "w") as f:
        json.dump(infarctus_myocarde, f, ensure_ascii=False, indent=1)
    print("infarctus_myocarde sections:", len(infarctus_myocarde["sections"]),
          "total blocks:", sum(len(s["items"]) for s in infarctus_myocarde["sections"]))

    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique", "fiche_monitorage_traumatise", "fiche_infarctus_myocarde"):
            del sys.modules[mn]
    avc_precoce = extract_avc_precoce()
    with open(os.path.join(BUILD_DIR, "content_avc_precoce.json"), "w") as f:
        json.dump(avc_precoce, f, ensure_ascii=False, indent=1)
    print("avc_precoce sections:", len(avc_precoce["sections"]),
          "total blocks:", sum(len(s["items"]) for s in avc_precoce["sections"]))

    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique", "fiche_monitorage_traumatise", "fiche_infarctus_myocarde", "fiche_avc_precoce"):
            del sys.modules[mn]
    traumatisme_cranien_grave_precoce = extract_traumatisme_cranien_grave_precoce()
    with open(os.path.join(BUILD_DIR, "content_traumatisme_cranien_grave_precoce.json"), "w") as f:
        json.dump(traumatisme_cranien_grave_precoce, f, ensure_ascii=False, indent=1)
    print("traumatisme_cranien_grave_precoce sections:", len(traumatisme_cranien_grave_precoce["sections"]),
          "total blocks:", sum(len(s["items"]) for s in traumatisme_cranien_grave_precoce["sections"]))
    ...
    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique", "fiche_monitorage_traumatise", "fiche_infarctus_myocarde", "fiche_avc_precoce", "fiche_traumatisme_cranien_grave_precoce"):
            del sys.modules[mn]
    examens_preinterventionnels = extract_examens_preinterventionnels()
    with open(os.path.join(BUILD_DIR, "content_examens_preinterventionnels.json"), "w") as f:
        json.dump(examens_preinterventionnels, f, ensure_ascii=False, indent=1)
    print("examens_preinterventionnels sections:", len(examens_preinterventionnels["sections"]),
          "total blocks:", sum(len(s["items"]) for s in examens_preinterventionnels["sections"]))

    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique", "fiche_monitorage_traumatise", "fiche_infarctus_myocarde", "fiche_avc_precoce", "fiche_traumatisme_cranien_grave_precoce", "fiche_examens_preinterventionnels"):
            del sys.modules[mn]
    aap_endoprotheses_coronaires = extract_aap_endoprotheses_coronaires()
    with open(os.path.join(BUILD_DIR, "content_aap_endoprotheses_coronaires.json"), "w") as f:
        json.dump(aap_endoprotheses_coronaires, f, ensure_ascii=False, indent=1)
    print("aap_endoprotheses_coronaires sections:", len(aap_endoprotheses_coronaires["sections"]),
          "total blocks:", sum(len(s["items"]) for s in aap_endoprotheses_coronaires["sections"]))

    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique", "fiche_monitorage_traumatise", "fiche_infarctus_myocarde", "fiche_avc_precoce", "fiche_traumatisme_cranien_grave_precoce", "fiche_examens_preinterventionnels", "fiche_aap_endoprotheses_coronaires"):
            del sys.modules[mn]
    recommandations_avk = extract_recommandations_avk()
    with open(os.path.join(BUILD_DIR, "content_recommandations_avk.json"), "w") as f:
    ...
    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique", "fiche_monitorage_traumatise", "fiche_infarctus_myocarde", "fiche_avc_precoce", "fiche_traumatisme_cranien_grave_precoce", "fiche_examens_preinterventionnels", "fiche_aap_endoprotheses_coronaires", "fiche_recommandations_avk", "fiche_douleur_postoperatoire"):
            del sys.modules[mn]
    ...
    for mn in list(sys.modules):
        if mn in ("fiche_ecbu", "style", "annexe_specialites", "fiche_anticoagulants", "fiche_choc_hemorragique", "fiche_intubation_urgence", "fiche_sepsis", "fiche_urgences_obstetricales", "fiche_anaphylaxie", "fiche_preeclampsie", "fiche_hyperthermie_maligne", "fiche_anticoag_urgence", "fiche_traumatisme_abdominal", "fiche_sedation_reanimation", "fiche_sedation_urgences", "fiche_vni", "fiche_aap_urgence", "fiche_curares", "fiche_remplissage", "fiche_traumatisme_membre", "fiche_voies_aeriennes_enfant", "fiche_intubation_difficile_adulte", "fiche_traumatisme_pelvien", "fiche_traumatisme_thoracique", "fiche_traumatisme_cranien", "fiche_traumatisme_vertebromedullaire", "fiche_intubation_reanimation", "fiche_traumatisme_cranien_leger", "fiche_lat_soins_critiques", "fiche_sdra", "fiche_pavm", "fiche_tracheotomie", "fiche_nutrition", "fiche_eer", "fiche_ira", "fiche_ih", "fiche_epanchement_pleural", "fiche_anemie", "fiche_hypothermie", "fiche_nvpo", "fiche_aap_programmee", "fiche_mtev_perioperatoire", "fiche_glycemie", "fiche_mal_epileptique", "fiche_allergie_prevention", "fiche_antibioprophylaxie", "fiche_controle_temperature", "fiche_tih", "fiche_civd", "fiche_eclsa", "fiche_transport_intrahospitalier", "fiche_transfusion_plasma", "fiche_sevrage_vm", "fiche_asthme_aigu_grave", "fiche_pancreatite", "fiche_corticotherapie", "fiche_antibiotherapie_probabiliste", "fiche_hsa", "fiche_sepsis_hemodynamique", "fiche_securisation_proc", "fiche_mort_encephalique", "fiche_monitorage_traumatise", "fiche_infarctus_myocarde", "fiche_avc_precoce", "fiche_traumatisme_cranien_grave_precoce", "fiche_examens_preinterventionnels", "fiche_aap_endoprotheses_coronaires", "fiche_recommandations_avk", "fiche_douleur_postoperatoire", "fiche_voies_aeriennes_adulte"):
            del sys.modules[mn]
    tih_2002 = extract_tih_2002()
    with open(os.path.join(BUILD_DIR, "content_tih_2002.json"), "w") as f:
        json.dump(tih_2002, f, ensure_ascii=False, indent=1)
    print("tih_2002 sections:", len(tih_2002["sections"]),
          "total blocks:", sum(len(s["items"]) for s in tih_2002["sections"]))

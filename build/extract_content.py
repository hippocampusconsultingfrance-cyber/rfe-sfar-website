...
def extract_gestion_traitements_chroniques_cardio_2009():
    import style
    import fiche_gestion_traitements_chroniques_cardio_2009 as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "gestion_traitements_chroniques_cardio_2009", "sections": blocks}


def extract_gestion_traitements_chroniques_douleur_toxico_2009():
    import style
    import fiche_gestion_traitements_chroniques_douleur_toxico_2009 as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
...
def extract_gestion_traitements_chroniques_infectieux_2009():
    import style
    import fiche_gestion_traitements_chroniques_infectieux_2009 as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "gestion_traitements_chroniques_infectieux_2009", "sections": blocks}


def extract_delivrance_information_2012():
    import style
    import fiche_delivrance_information_2012 as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "delivrance_information_2012", "sections": blocks}


def extract_blocs_perimedullaires_ci_2006():
    import style
    import fiche_blocs_perimedullaires_ci_2006 as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
...
def extract_blocs_perimedullaires_cesarienne_2006():
    import style
    import fiche_blocs_perimedullaires_cesarienne_2006 as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "blocs_perimedullaires_cesarienne_2006", "sections": blocks}


def extract_blocs_perimedullaires_postop_2006():
    import style
    import fiche_blocs_perimedullaires_postop_2006 as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "blocs_perimedullaires_postop_2006", "sections": blocks}


def extract_blocs_perimedullaires_terrain_cardioresp_2006():
    import style
    import fiche_blocs_perimedullaires_terrain_cardioresp_2006 as m
    trace = []
    make_module_patches(m, trace)
    make_module_patches(style, trace)
    ...
    for title, fn in m.SECTIONS:
        ...
        for x in items:
            ...
            if isinstance(r, list):
                ...
                resolved.append(r)
        blocks.append({"title": title, "items": resolved})
    return {"doc": "blocs_perimedullaires_terrain_cardioresp_2006", "sections": blocks}
...
if __name__ == "__main__":
    ...
    for mn in list(sys.modules):
        if mn.startswith("fiche_") or mn in ("style", "annexe_specialites"):
            del sys.modules[mn]
    gestion_traitements_chroniques_cardio_2009 = extract_gestion_traitements_chroniques_cardio_2009()
    with open(os.path.join(BUILD_DIR, "content_gestion_traitements_chroniques_cardio_2009.json"), "w") as f:
        json.dump(gestion_traitements_chroniques_cardio_2009, f, ensure_ascii=False, indent=1)
    print("gestion_traitements_chroniques_cardio_2009 sections:", len(gestion_traitements_chroniques_cardio_2009["sections"]),
          "total blocks:", sum(len(s["items"]) for s in gestion_traitements_chroniques_cardio_2009["sections"]))

    for mn in list(sys.modules):
        if mn.startswith("fiche_") or mn in ("style", "annexe_specialites"):
            del sys.modules[mn]
    gestion_traitements_chroniques_douleur_toxico_2009 = extract_gestion_traitements_chroniques_douleur_toxico_2009()
    with open(os.path.join(BUILD_DIR, "content_gestion_traitements_chroniques_douleur_toxico_2009.json"), "w") as f:
    ...
    for mn in list(sys.modules):
        if mn.startswith("fiche_") or mn in ("style", "annexe_specialites"):
            del sys.modules[mn]
    gestion_traitements_chroniques_infectieux_2009 = extract_gestion_traitements_chroniques_infectieux_2009()
    with open(os.path.join(BUILD_DIR, "content_gestion_traitements_chroniques_infectieux_2009.json"), "w") as f:
        json.dump(gestion_traitements_chroniques_infectieux_2009, f, ensure_ascii=False, indent=1)
    print("gestion_traitements_chroniques_infectieux_2009 sections:", len(gestion_traitements_chroniques_infectieux_2009["sections"]),
          "total blocks:", sum(len(s["items"]) for s in gestion_traitements_chroniques_infectieux_2009["sections"]))

    for mn in list(sys.modules):
        if mn.startswith("fiche_") or mn in ("style", "annexe_specialites"):
            del sys.modules[mn]
    delivrance_information_2012 = extract_delivrance_information_2012()
    with open(os.path.join(BUILD_DIR, "content_delivrance_information_2012.json"), "w") as f:
        json.dump(delivrance_information_2012, f, ensure_ascii=False, indent=1)
    print("delivrance_information_2012 sections:", len(delivrance_information_2012["sections"]),
          "total blocks:", sum(len(s["items"]) for s in delivrance_information_2012["sections"]))

    for mn in list(sys.modules):
        if mn.startswith("fiche_") or mn in ("style", "annexe_specialites"):
            del sys.modules[mn]
    blocs_perimedullaires_ci_2006 = extract_blocs_perimedullaires_ci_2006()
    with open(os.path.join(BUILD_DIR, "content_blocs_perimedullaires_ci_2006.json"), "w") as f:
    ...
    for mn in list(sys.modules):
        if mn.startswith("fiche_") or mn in ("style", "annexe_specialites"):
            del sys.modules[mn]
    blocs_perimedullaires_cesarienne_2006 = extract_blocs_perimedullaires_cesarienne_2006()
    with open(os.path.join(BUILD_DIR, "content_blocs_perimedullaires_cesarienne_2006.json"), "w") as f:
        json.dump(blocs_perimedullaires_cesarienne_2006, f, ensure_ascii=False, indent=1)
    print("blocs_perimedullaires_cesarienne_2006 sections:", len(blocs_perimedullaires_cesarienne_2006["sections"]),
          "total blocks:", sum(len(s["items"]) for s in blocs_perimedullaires_cesarienne_2006["sections"]))

    for mn in list(sys.modules):
        if mn.startswith("fiche_") or mn in ("style", "annexe_specialites"):
            del sys.modules[mn]
    blocs_perimedullaires_postop_2006 = extract_blocs_perimedullaires_postop_2006()
    with open(os.path.join(BUILD_DIR, "content_blocs_perimedullaires_postop_2006.json"), "w") as f:
        json.dump(blocs_perimedullaires_postop_2006, f, ensure_ascii=False, indent=1)
    print("blocs_perimedullaires_postop_2006 sections:", len(blocs_perimedullaires_postop_2006["sections"]),
          "total blocks:", sum(len(s["items"]) for s in blocs_perimedullaires_postop_2006["sections"]))

    for mn in list(sys.modules):
        if mn.startswith("fiche_") or mn in ("style", "annexe_specialites"):
            del sys.modules[mn]
    blocs_perimedullaires_terrain_cardioresp_2006 = extract_blocs_perimedullaires_terrain_cardioresp_2006()
    with open(os.path.join(BUILD_DIR, "content_blocs_perimedullaires_terrain_cardioresp_2006.json"), "w") as f:
        json.dump(blocs_perimedullaires_terrain_cardioresp_2006, f, ensure_ascii=False, indent=1)
    print("blocs_perimedullaires_terrain_cardioresp_2006 sections:", len(blocs_perimedullaires_terrain_cardioresp_2006["sections"]),
          "total blocks:", sum(len(s["items"]) for s in blocs_perimedullaires_terrain_cardioresp_2006["sections"]))

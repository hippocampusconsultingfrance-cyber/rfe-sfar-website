...
def _section_tc():
    ...
    story.append(Spacer(1, 2*mm))
    story.append(P(
        "<b>Précisions :</b> R2.1/R2.2, échec du concept de neuroprotection par hypothermie "
        "chez le TC grave — aucun bénéfice démontré de l'hypothermie thérapeutique (32-35 °C) "
        "versus normothermie sur la mortalité ou le devenir neurologique, d'où le CCT plus "
        "modéré (35-37 °C) ciblant la seule prévention de l'hyperthermie.", S_NOTE))
    return story
...
def _section_autres():
    ...
    story.append(Spacer(1, 2*mm))
    story.append(P(
        "<b>Précisions :</b> R4.2/R4.3 vs R4.4 — la conduite s'inverse selon la présence "
        "d'une HTIC associée (pas de CCT si fièvre bien tolérée et pas d'HTIC ; hypothermie "
        "recommandée si HTIC associée).", S_NOTE))
    return story
...
def _section_modalites():
    ...
    story.append(Spacer(1, 2*mm))
    story.append(P(
        "<b>Précisions :</b> R6.3, bon accord entre sites centraux (artère pulmonaire, "
        "œsophage, vessie, rectum — le rectum étant le seul site périphérique validé), "
        "accord médiocre pour les sites périphériques cutané/tympanique (biais rapporté de "
        "1 °C). R6.4, hypokaliémie &lt;3,5 mmol/L et risque accru d'arythmie à surveiller.",
        S_NOTE))
    return story
...
def _count_pages(story_flowables):
    ...
    # NOTE: pypdf/cryptography is broken in this container (pyo3 panic on import) - use
    # PyMuPDF (fitz) instead.

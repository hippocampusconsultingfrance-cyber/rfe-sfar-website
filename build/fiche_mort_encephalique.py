...
source) : 1-3 = "desaccord", 4-6 = "indecision", 7-9 = "accord".
CORRECTION POST-AUDIT INDEPENDANT : une premiere lecture de ce script avait interprete
le motif imprime "(8,5)" / "(7,5)" comme "mediane, extreme dissident cite" (deux
valeurs distinctes) et l'avait rendu sous forme de chip a deux nombres "8,5" / "7,5".
Un audit independant a l'aveugle (relecture du seul texte source, sans acces a ce
script) a signale que cette lecture etait statistiquement improbable : le second
chiffre est TOUJOURS "5" sur les 11 occurrences (4x "8,5", 7x "7,5" - grep exhaustif
ci-dessous), alors qu'un extreme reellement cite sur une echelle 1-9 varierait d'une
question a l'autre. La lecture correcte, conforme a la notation decimale francaise
(virgule = point decimal), est que "(8,5)" et "(7,5)" sont des MEDIANES A UN DEMI-POINT
(8,5 et 7,5) - consequence normale d'une mediane calculee sur un nombre pair de cotations
entieres. Cette fiche a ete corrigee pour reproduire un chip a valeur unique "8,5" /
"7,5" (jamais scinde en deux nombres, jamais un extreme invente) ; les deux valeurs
restent en zone "accord" (7-9).
...
def _section_intro():
    ...
    story.append(info_panel(P(
        ...
        "vérifié cette lecture par grep exhaustif : le second chiffre après la virgule "
        "est <i>toujours</i> « 5 » sur les 11 occurrences du document, ce qui exclut "
        "l'hypothèse d'un extrême dissident cité (qui varierait). Chip à valeur unique "
        "« 8,5 », jamais scindé en deux nombres ni interprété comme un second vote.<br/><br/>"
